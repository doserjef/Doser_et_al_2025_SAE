# 6e-hold-out-predict.R: script to predict county-level biomass for each of the 20 species
#                        across the southeastern US for the hold-out counties.  
...
# TODO:
# indx <- 1
if(length(args) == 0) base::stop('Need to tell me the data set')
print(indx)
...
if (machine.name == 'pop-os') {
  ...
  data.dir <- '/share/doserlab/jwdoser/DIDF24/data/cross_val_data/'
}
...
# Load Stage 1 data -------------------------------------------------------
load(paste0(data.dir, 'hold_', indx, '_stage_1_data_validation_fit.rda'))
tmin.mean <- mean(data.fit.1$covs$tmin)
tmin.sd <- sd(data.fit.1$covs$tmin)
tmax.mean <- mean(data.fit.1$covs$tmax)
tmax.sd <- sd(data.fit.1$covs$tmax)
ppt.mean <- mean(data.fit.1$covs$ppt)
ppt.sd <- sd(data.fit.1$covs$ppt)

# Load Stage 2 data -------------------------------------------------------
load(paste0(data.dir, 'hold_', indx, '_stage_2_data_validation_fit.rda'))
tcc.mean <- mean(data.fit.2$covs$tcc)
tcc.sd <- sd(data.fit.2$covs$tcc)
vpd.mean <- mean(data.fit.2$covs$vpd)
vpd.sd <- sd(data.fit.2$covs$vpd)
ppt.mean <- mean(data.fit.2$covs$ppt)
ppt.sd <- sd(data.fit.2$covs$ppt)
elev.mean <- mean(data.fit.2$covs$elev)
elev.sd <- sd(data.fit.2$covs$elev)

# Load resulting model objects --------------------------------------------
# Stage 1
load(paste0(out.dir, 'se-validation-stage-1-2e+05-samples-5-factors-', indx, '-fold-2025-07-11.rda'))
...
gc()
# Stage 2
load(paste0(out.dir, 'se-validation-stage-2-1e+05-samples-5-factors-', indx, '-fold-2025-07-09.rda'))
...
load(paste0(data.dir, 'hold_', indx, '_prediction_data.rda'))
...
save(biomass.mean.samples, file = paste0(out.dir, 'hold-', indx, '-biomass-sae-county-means.rda'))
