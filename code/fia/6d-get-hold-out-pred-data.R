# 6d-get-hold-out-pred-data.R: script to extract the coordinates and covariates across
#                              the hold out counties for assessment of the presented 
#                              modeling framework's ability to draw reliable inferences 
#                              in counties not used to fit the model.
...
# Number of folds (NOTE: hardcoded)
n.folds <- 4

# Generate the prediction data set for each hold out set one at a time ----
for (l in 1:n.folds) {
  print(paste0("Currently on fold ", l, " out of ", n.folds)) 
  # Load hold out data to get held out counties -----------------------------
  load(paste0('data/cross_val_data/hold_', l, '_stage_2_data_validation_hold.rda'))  
  coords.hold <- data.hold.2$coords
 
  # Determine the counties that each plot falls into 
  ...
  se.county <- usa.county %>%
    ...
    st_as_sf()
  coords.sf <- st_as_sf(data.frame(data.hold.2$coords),
                        coords = c('X', 'Y'),
                        crs = my.crs)
  ...
  n.plots <- nrow(data.hold.2$coords)
  county <- vector(mode = 'character', length = n.plots)
  for (i in 1:n.counties) {
    county[indx.by.county[[i]]] <- se.county$county[i]
  }
  # Unique counties in the hold out data set
  unique.counties <- unique(county)
  # Filter se.county to only include those counties in the hold out data
  se.county <- se.county %>%
    filter(county %in% unique.counties)
  
  # Generate prediction grid and data set -----------------------------------
  # The dx x dy indicates the resolution in terms of km. Predicting at a 1x1km resolution.
  ...
  coords.pred.sf <- st_intersection(coords.pred.sf, st_make_valid(se.county))
  ...
  ## Citation: Terrain Tiles was accessed on 4 July 2025 from https://registry.opendata.aws/terrain-tiles.
  ...
  # Remove sites with any NA values or sites with TCC > 255
  ...
  county <- vector('character', nrow(X.0))
  indx.by.county <- st_contains(se.county, coords.pred.sf)
  n.counties <- nrow(se.county)
  # Fill in county vector with the appropriate information
  for (i in 1:n.counties) {
    county[indx.by.county[[i]]] <- se.county$county[i]
  ...
  save(X.0, coords.0, county, 
       file = paste0('data/cross_val_data/hold_', l, '_prediction_data.rda'))
}
